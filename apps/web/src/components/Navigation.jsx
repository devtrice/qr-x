    ...
    <Link
      ...
      className={clsx(
        ...
        active ?
          ...
          'text-zinc-600 hover:text-blue-500 dark:text-zinc-400 dark:hover:text-white',
      )}
    >
...
function VisibleSectionHighlight({group, pathname}) {
  ...
  const height = isPresent ?
    Math.max(1, visibleSections.length - 20) * itemHeight  :
    itemHeight;
  const top =
...
function ActivePageMarker({group, pathname}) {
  ...
  return (
    <motion.div
      layout
      className="absolute w-px h-6 bg-blue-500 left-2"
      initial={{opacity: 0}}
      animate={{opacity: 1, transition: {delay: 0.2}}}
      exit={{opacity: 0}}
      style={{top}}
...
function NavigationGroup({group, className}) {
  ...
  return (
    <li className={clsx('relative mt-8', className)}>
      <motion.h2
        layout="position"
...
export const navigation = [
...

  {
    title: 'Accessibility',
    links: [
      {title: 'Screen Readers', href: '/docs/screen'},
      {title: 'Official Plugin', href: '/docs/official-plugin'},
    ],
  },
];
      ...
      <ul role="list">
        ...
        {navigation.map((group, groupIndex) => (
          <NavigationGroup
            key={group.title}
            group={group}
            className={groupIndex === 0 && 'md:my-2'}
          />
        ))}
      </ul>
    </nav>
